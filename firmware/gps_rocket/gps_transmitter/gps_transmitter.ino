...
void SDfile() {
  ...
  char filename[] = "VSD.CSV"; //very special data
      ...
      f.print(",");
      f.print(orientationToString(o)); f.print(",");

      //GPS write
      f.print(p.fix); f.print(",");
...
void readGPS(){
  for (int i = 0; i < 64 && GPS.available(); i++) {
    GPS.read();
  }

  // If a full sentence is ready, parse it
  if (GPS.newNMEAreceived()) {
    char* nmea = GPS.lastNMEA();
    GPS.parse(nmea);
    
    // Immediately update our payload when new data arrives
    p.fix = (uint8_t)GPS.fix;
    p.sats = (uint8_t)GPS.satellites;
...
void setup() {
  ...
  if (!GPS.begin(0x10)) {
    Serial.println("GPS NOT FOUND - check STEMMA cord");
    while(1); 
  }

  GPS.sendCommand("$PMTK353,1,1,1,0,0*2A"); // GPS, GLO, GAL only
  ...
  if (f) {
    // CSV header
    f.println("time_ms,raw_x,raw_y,raw_z,accel_x,accel_y,accel_z,orientation,fix,sats,lat,lon");
    f.close();
  }
...
uint32_t llt = 0;
void loop() {
  // 1. Run this as fast as the CPU allows
  readGPS();
  yield();
